...
export const ACTION_SPECS = {
  ...
  // ---------------------------------------------------------------- garden & crops (§4.5)
  PlantSeed: { wire: 'PlantSeed', form: 'wrapped', category: 'garden', params: ['slot', 'species'] },
  WaterPlant: { wire: 'WaterPlant', form: 'wrapped', category: 'garden', params: ['slot'] },
  HarvestCrop: {
    wire: 'HarvestCrop',
    form: 'wrapped',
    category: 'garden',
    params: ['slot', 'slotsIndex', 'cropItemId'],
    note: 'slotsIndex is the crop to harvest (its own slotId) and is required by the bundle schema. cropItemId is a client-minted UUID and is required; mint it, do not wait for one. The resulting item arrives in the next inventory patch (a RoomFrame since v756; PartialState before that); there is no synchronous response.',
  },
